...
async def main() -> None:
    ...
    try:
        if args.command == 'upload':
            magnet = await cli.upload_file(args.file, args.temp)
            print(f"Magnet link: {magnet}")
            
        elif args.command == 'download':
            output_path = await cli.download_file(args.magnet, args.output, args.temp)
            ...
            if peers:
                ...
                for peer in peers:
                    ...
                    print(f"  Files: {len(peer['shared_files'])}")
            else:
